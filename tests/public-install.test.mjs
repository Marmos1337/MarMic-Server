...
test('bootstrap itself uses bounded retries and atomic partial downloads', () => {
  ...
  assert.match(source, /VERSION="0\.13\.5"/u);
...
case "$output" in
  *.sha256.part) printf '${sha}  marmic-server-0.13.5-linux-amd64.tar.gz\n' > "$output" ;;
...
`,
  ...
  writeExecutable(
    ...
    `#!/bin/sh
if [ "$1" = '-tzf' ]; then
  printf 'marmic-server-0.13.5-linux-amd64/\n'
  printf 'marmic-server-0.13.5-linux-amd64/manifest.json\n'
  exit 0
fi
...
mkdir -p "$destination/marmic-server-0.13.5-linux-amd64"
printf '{"version":"0.13.5","architecture":"amd64","sourceCommit":"test"}\n' > "$destination/marmic-server-0.13.5-linux-amd64/manifest.json"
`,
  );
  writeExecutable(
    join(bin, 'node'),
